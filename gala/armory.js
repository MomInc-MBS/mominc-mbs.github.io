...
function paintTiers(){const p=P.read();$('weapon-tiers').replaceChildren();W.tiers.forEach((name,index)=>{const b=document.createElement('button'),w={type,tier:index},open=W.unlocked(w,p);b.type='button';b.className='weapon-tier';b.dataset.tier=String(index);b.dataset.locked=String(!open);b.setAttribute('aria-label',W.name(w)+(open?', unlocked':', locked preview'));b.append(canvas(w));const label=document.createElement('span');label.textContent=index===0?'Level 1':`Upgrade ${index}`;const state=document.createElement('small');state.textContent=open?'Unlocked':'Locked';b.append(label,state);b.onclick=()=>{tier=index;detail();};$('weapon-tiers').append(b);});detail();}
function refresh(){if(!api)return;detail();}
function mount(controller){api=controller;
 for(const family of W.types){const b=document.createElement('button');b.type='button';b.className='weapon-type';b.dataset.type=family.id;b.append(canvas({type:family.id,tier:0},24,44));const label=document.createElement('span');label.textContent=family.name;b.append(label);b.onclick=()=>{type=family.id;tier=api.getLook().weapon?.type===type?api.getLook().weapon.tier:0;paintTiers();api.selectCategory?.('upgrades');};$('weapon-types').append(b);}
