...
window.GalaWeaponMotion={drawAnimatedWeapon,evolution,abilityFor,activate:activateGalaSpecial,remaining:()=>cooldown.remaining()};
...
export function mountWeaponStation(host,{getWeapon,getProgress=()=>window.GalaProgress.read(),canUse=()=>true,choose=false}={}){
 ...
 if(choose){
  ...
  for(const item of W.types){const option=document.createElement('option');option.value=item.id;option.textContent=item.name;family.append(option);}
 ...
 function refresh(){
  const value=weapon(),p=getProgress(),profile=evolution(value),key=value.type+':'+value.tier+':'+p.activeDays+':'+p.strength;
  if(key!==signature){signature=key;action=null;host.style.setProperty('--weapon-energy',profile.energy);$('[data-name]').textContent=W.name(value);const r=W.requirements(value);$('[data-requirement]').textContent=W.unlocked(value,p)?(profile.ability?`${profile.ability.cooldownMs/1000}s cooldown · Rank ${profile.ability.rank}`:'Special unlocks at tier 4'):`${r.days} workout days · Strength ${r.strength}`;
