...
function normalize(raw){if(!raw||raw.schema!=='mominc-avatar'||raw.version!==1||!raw.parts)throw Error('Choose a MOM Inc avatar file.');const n={schema:'mominc-avatar',version:1,name:typeof raw.name==='string'?raw.name.trim().slice(0,64):'',dye:raw.dye,parts:{}};if(!Number.isInteger(n.dye)||n.dye<0||n.dye>9)throw Error('This look has an unknown silk colour.');for(const s of sections){let v=raw.parts[s.id]??(s.id==='pet'?0:undefined);if(!Number.isInteger(v)||v<0||v>=s.names.length)throw Error('This look has an unknown wardrobe piece.');n.parts[s.id]=v;}if(raw.weapon!=null){if(!window.GalaWeapons)throw Error('Open the updated Gala to import this weapon.');n.weapon=window.GalaWeapons.normalize(raw.weapon);}return n;}
...
function draw(canvas,look,{base=true,weapon=true,time=0,companion=true,petOnly=false,prop=true,blink=false,pose={}}={}){
 ...
 const armPose=side=>{const x=side<0?21-width:44+width,y=39;let angle=(pose.walk||0)*side*.1,stretch=1;if(side<0&&pose.petting!=null){stretch=[1.8,1.5,1.02,1.55,1.5,1.4,1.5,1.4,1.25,1.16][p.pet];angle=Math.asin((x-14)/(23*stretch))+pose.petting*.045;}if(side>0&&pose.weapon)angle=-.8;c.translate(x,y);c.rotate(angle);c.scale(1,stretch);c.translate(-x,-y);};
 if(!petOnly){
 ...
 c.save();armPose(-1);poly([[23-width,36],[18-width,39],[16-width,58],[22-width,59],[26-width,43]],S);c.restore();c.save();armPose(1);poly([[41+width,36],[46+width,39],[48+width,58],[42+width,59],[38+width,43]],D);c.restore();
 ...
 // Face features.
 let f=p.face;const eye=(x,y,w=3)=>{if(blink){rect(x,y+2,w+2,1,ink);return;}box(x,y,w+2,4,white);rect(x+2,y+1,1,2,ink);};
 if(f===1){if(blink){rect(26,24,12,1,ink);}else{box(26,21,12,7,white);box(30,22,5,5,'#926299');dot(31,23,white);}}else if(f===3){box(24,21,16,5,ink);rect(25,22,13,1,blink?B:H);}else if(f===5){poly([[24,20],[29,22],[29,27],[24,25]],ink);poly([[39,20],[34,22],[34,27],[39,25]],ink);if(blink){rect(24,20,6,7,S);rect(34,20,6,7,S);rect(24,24,5,1,ink);rect(35,24,5,1,ink);}else{dot(25,22,white);dot(36,22,white);}}else if(f===8){poly([[24,20],[32,22],[40,20],[38,28],[26,28]],gold);rect(26,23+(blink?1:0),3,blink?1:2,ink);rect(35,23+(blink?1:0),3,blink?1:2,ink);}else{eye(25,22);eye(34,22);if(f===2)eye(29,17);if(f===4){eye(25,17,2);eye(35,17,2);}if(f===9)for(const [x,y] of [[26,17],[34,17],[26,27],[34,27]])eye(x,y,1);}
 ...
 // Party props remain independent from the earned floating weapon.
 if(prop)switch(p.held){case 0:box(48+width,47,5,8,'#c4bdcf');rect(50+width,55,1,8,gold);rect(48+width,63,5,1,gold);rect(49+width,49,3,3,H);break;case 1:poly([[48,59],[43,46],[49,40],[58,45],[60,53]],H);for(let i=0;i<4;i++)poly([[48,59],[45+i*4,44+i%2]],gold);break;case 2:box(46,56,13,9,H);rect(47,59,11,1,gold);gem(53,59);break;case 3:poly([[49,63],[45,47],[56,47],[51,63]],'#548267');for(const [x,y] of [[45,45],[51,42],[57,46],[49,49]])gem(x,y,H);break;case 4:rect(51,47,2,36,gold);box(47,45,7,3,gold);break;case 5:rect(51,47,1,6,gold);box(47,53,10,13,B);rect(49,56,6,7,gold);gem(52,59,white);break;case 6:box(47,53,12,9,white);rect(49,55,7,1,B);gem(53,59,H);break;case 7:box(48,47,9,9,H);gem(52,51,white);rect(50,56,5,2,gold);break;case 8:rect(52,29,1,35,gold);poly([[40,33],[43,25],[52,22],[62,29],[63,33]],H);rect(42,32,20,1,gold);break;case 9:poly([[48,48],[48,43],[51,46],[56,44],[59,48],[57,55],[50,54]],S);dot(51,49,ink);dot(56,49,ink);rect(51,55,1,4,gold);rect(56,55,1,4,gold);}
 // Three additional collections give every part new cuts or surface details.
 for(const section of sections){const g=group(section.id);if(!g||['skin','pet'].includes(section.id)||(!prop&&section.id==='held'))continue;const accent=g===1?gold:g===2?white:'#88e4d9';const shift=(original[section.id]%10)%3;
 switch(section.id){
 ...
 case 'arms':for(const x of [19-width,42+width]){c.save();armPose(x<32?-1:1);if(g===1)rect(x,44,2,10,accent);if(g===2)poly([[x,43],[x-4,54],[x+4,54]],B);if(g===3)for(let y=44;y<55;y+=4)gem(x+1,y,accent);c.restore();}break;
 case 'hands':for(const x of [19-width,42+width]){c.save();armPose(x<32?-1:1);if(g===1)gem(x+2,58,accent);if(g===2)for(let k=0;k<3;k++)rect(x+k*2,62,1,4,accent);if(g===3)box(x-1,58,7,3,accent);c.restore();}break;
 ...
 if(companion&&original.pet){const k=p.pet,g=group('pet'),fur=[H,S,gold,'#90bca2','#d99bae','#ad9ac8','#81a891','#b1accb','#9caabb','#83b587'][k],x=9,y=k===2?67:79;
