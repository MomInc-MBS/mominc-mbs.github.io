/* The 2048 table keeps its game/input; its room is now a specimen-control terminal. */
(()=>{'use strict';
 ...
 function mount(board){
  const hud=document.createElement('aside');hud.className='gala-war-hud';hud.innerHTML='<div class="gala-war-title">MOM_OS 95 <span>4 × 4 / WAR ROOM</span></div><small>Merge to $2048 · $16 / $32 starting tiles</small><strong data-war-name>Anonymous guest</strong><nav><a href="/gala/terminal/" target="_top">Rankings terminal ↗</a><a href="/play/djscratch/" target="_top">DJ identity ↗</a></nav>';document.body.append(hud);
  ...
  // The original tray geometry and all numbered tiles stay in place.

  return {dispose(){disposed=true;hologram?.dispose();hud.remove();host.remove();window.removeEventListener('storage',identity);window.removeEventListener('mbs:dj-identity',identity);}};
 }
