...
(()=>{'use strict';
 function dress(scene,T){
  ...
  // The projector and its specimen share the old stress gauge's place beside the board.
  const projector=new T.Group();projector.name='myr5-projector';projector.position.set(2.9,0,0);projector.userData.HologramMaterial=T.MeshBasicMaterial;scene.add(projector);
  const ring=new T.Mesh(new T.CylinderGeometry(.54,.65,.18,40),edge);ring.position.y=-.1;ring.receiveShadow=true;ring.castShadow=true;projector.add(ring);
  const lens=new T.Mesh(new T.CylinderGeometry(.46,.46,.025,40),glow);lens.position.y=.005;projector.add(lens);
 ...
 function mount(board){
  const hud=document.createElement('aside');hud.className='gala-war-hud';hud.innerHTML='<div class="gala-war-title">RESISTANCE <span>AMMO FOUNDRY</span></div><p>Merge matching ammo. Double its firepower.</p><strong data-war-tier>Pellet · +1</strong><p data-war-next>Next: Cartridge · +2</p><p data-war-gain role="status" aria-live="polite">Small beginnings. Impossible firepower.</p><small data-war-name>Anonymous guest</small><nav><a href="/play/goon/" target="_top">Original Gala ↗</a><a href="/gala/terminal/" target="_top">Rankings ↗</a></nav>';document.body.append(hud);
  const projector=board.scene.getObjectByName('myr5-projector');
  if(board.bar)board.bar.visible=false;
  const status=document.createElement('span');status.className='gala-hologram-status';status.setAttribute('role','status');hud.append(status);let hologram=null,disposed=false;
  import('/gala/hologram.mjs?v=projector2').then(m=>m.mountSceneHologram(projector,{Material:projector.userData.HologramMaterial,onStatus:text=>{status.textContent=text;}})).then(value=>{if(disposed)value?.dispose();else hologram=value;}).catch(()=>{status.textContent='MYR5 / SIGNAL LOST';});
  ...
  // The old camera cropped the gauge on narrow screens. Keep the board and the
  // whole projector in view after the game's own resize has placed the camera.
  let fitFrame=0;
  function fitCamera(){
   fitFrame=0;if(disposed)return;const camera=board.camera,point=camera.position.clone();camera.zoom=1;camera.updateProjectionMatrix();camera.updateMatrixWorld();let extent=0;
   for(const box of [[[-2.2,2.2],[-.3,1.4],[-2.2,2.2]],[[2.25,3.55],[-.2,2.5],[-.65,.65]]])for(const x of box[0])for(const y of box[1])for(const z of box[2]){
    point.set(x,y,z).project(camera);extent=Math.max(extent,Math.abs(point.x),Math.abs(point.y));
   }
   camera.zoom=Math.min(1,.94/extent);camera.updateProjectionMatrix();
  }
  function queueFit(){if(fitFrame)cancelAnimationFrame(fitFrame);fitFrame=requestAnimationFrame(fitCamera);}
  window.addEventListener('resize',queueFit,{passive:true});queueFit();
  ...
  return {sync,dispose(){disposed=true;if(fitFrame)cancelAnimationFrame(fitFrame);hologram?.dispose();hud.remove();window.removeEventListener('resize',queueFit);window.removeEventListener('storage',identity);window.removeEventListener('mbs:dj-identity',identity);}};
 }
