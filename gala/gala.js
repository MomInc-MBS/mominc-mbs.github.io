...
function paint(){A.draw($('avatar'),current);$('avatar').setAttribute('aria-label',(current.name||'Your alien')+' in '+A.sections.find(s=>s.id==='torso').names[current.parts.torso]);if(document.activeElement!==$('guest-name'))$('guest-name').value=current.name;$('undo').disabled=!undo.length;$('redo').disabled=!redo.length;paintMenus();focusPreview();for(const [i,b] of [...$('dyes').children].entries())b.setAttribute('aria-pressed',String(i===current.dye));paintOptions();window.GalaArmory?.refresh();}
