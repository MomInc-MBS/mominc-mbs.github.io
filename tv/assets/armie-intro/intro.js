...
function valid(r){return r&&r.version===1&&r.complete===true&&r.source==='handborne'&&regions.every(k=>Number.isInteger(r.sections?.[k])&&r.sections[k]>=0&&r.sections[k]<20);}
async function loadHand(r){if(!valid(r))throw Error('Finish your hand in Handborne first.');recipe=r;const families=new Map();for(const id of new Set(Object.values(r.sections)))families.set(id,await loader.loadAsync('./models/family-'+String(id).padStart(2,'0')+'.glb'));for(const key of regions){const part=families.get(r.sections[key]).scene.getObjectByName(key);if(!part)throw Error('Missing hand region: '+key);const clone=part.clone(true);clone.userData.region=key;clone.userData.styleId=r.sections[key];hand.add(clone);}const bounds=new THREE.Box3().setFromObject(hand),center=bounds.getCenter(new THREE.Vector3()),size=bounds.getSize(new THREE.Vector3());handRig=bindHand(hand);hand.scale.setScalar(.95/Math.max(size.x,size.y,size.z));hand.rotation.set(-.4,0,-.55);hand.visible=false;hand.userData.recipe=r;ready=true;window.armieIntroReady=true;if(liveMode)parent.postMessage({type:'armie-intro-ready'},location.origin);summary.textContent='Your seven chosen hand parts are ready. Pull the drive. Coach must follow.';startButton.hidden=false;renderAt(0);if(new URLSearchParams(location.search).has('autoplay'))play();}
