...
import {MATERIAL_NOTES} from './creator/material-language';
import {CoachingPanel} from './coaching-panel';
import {EYE_LAYOUTS,type EyeLayout} from './eye-layouts';
import {PUPILS,type PupilShape} from './pupils';
import {fresh,parseRecipe,STYLES,PICKER_STYLES,REGIONS,LABELS,type Region,type Design} from './design';
...
export default function Home(){
 ...
 const selectPart=(region:Region)=>{setPlaying(false);setSelected(region);if(region===selected)viewer.current?.focus();};
 ...
 const pick=(id:number)=>commit({...design,styles:{...design.styles,[selected]:id}});
 ...
 const remix=()=>{if(locks.length===REGIONS.length){setMessage('Unlock a part to remix.');return;}const nums=crypto.getRandomValues(new Uint32Array(6));commit({...design,styles:Object.fromEntries(REGIONS.map((r,i)=>[r,locks.includes(r)?design.styles[r]:PICKER_STYLES[nums[i]%PICKER_STYLES.length].id])) as Design['styles']});};
 ...
 return <main className="editor-shell">
  ...
  <div className="editor-workspace">
   <section className="preview-bay" aria-label="Live coach preview">
    ...
    <div className="preview-stage"><AlienViewer ref={viewer} design={design} selected={selected} hologram={holo} parts={parts} playing={playing} onPick={r=>{selectPart(r);setTab('materials');}} onReady={()=>setReady(true)}/></div>
   ...
   <section className="design-console" aria-label="Coach design controls">
    <Tabs value={tab} onValueChange={chooseTab} className="console-tabs"><TabsList className="menu-tabs" aria-label="Customizer menus">{[['materials','Materials'],['face','Face'],['body','Body'],['coach','Coach'],['files','Files']].map(([id,label])=><TabsTrigger key={id} value={id}>{label}</TabsTrigger>)}</TabsList>
     <div className="console-scroll">
      <TabsContent value="materials"><div className="part-rail" aria-label="Creature parts">{REGIONS.map(r=><button key={r} aria-pressed={selected===r} onClick={()=>selectPart(r)}><i style={{background:STYLES[design.styles[r]].primary}}/>{SHORT[r]}{locks.includes(r)&&<Lock aria-label="Locked" size={12}/>}</button>)}</div>
       ...
       <p className="help">{MATERIAL_NOTES[style.id]}</p><div className="material-grid" aria-label="Surface styles">{PICKER_STYLES.map(s=><button key={s.name} aria-pressed={design.styles[selected]===s.id} onClick={()=>pick(s.id)}><img src={`/tv/assets/armie-intro/myr5/styles/${String(s.id).padStart(2,'0')}.png`} alt="" loading="lazy"/><span>{s.name}</span></button>)}</div>
      ...
      <TabsContent value="face"><div className="panel-heading"><div><small>SHAPE & EXPRESSION</small><h2>Face</h2></div></div><div className="field-grid"><label>Eye arrangement<select onFocus={()=>selectPart('eye')} id="eye-layout" value={design.eyeLayout} onChange={e=>commit({...design,eyeLayout:e.target.value as EyeLayout})}>{Object.entries(EYE_LAYOUTS).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}</select></label><label>Expression<select onFocus={()=>selectPart('eye')} value={design.eye} onChange={e=>commit({...design,eye:e.target.value as Design['eye']})}><option value="open">Curious</option><option value="sleepy">Unimpressed</option><option value="wide">Wide awake</option></select></label><label>Pupil shape<select onFocus={()=>selectPart('eye')} value={design.pupil} onChange={e=>commit({...design,pupil:e.target.value as PupilShape})}>{PUPILS.map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label></div><div className="range-stack">{range('iris','Iris size',.7,1.25)}{range('pupilSize','Pupil size',.6,1.15)}</div><p className="help">Use Back or drag the coach to inspect eyes behind the head.</p></TabsContent>
