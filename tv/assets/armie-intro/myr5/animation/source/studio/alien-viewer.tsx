...
export type ViewerHandle={exportFile:(format:'glb'|'obj'|'png')=>Promise<void>;front:()=>void;back:()=>void;focus:()=>void};
...
type Engine={scene:T.Scene;camera:T.PerspectiveCamera;renderer:T.WebGLRenderer;orbit:OrbitControls;assembly:Assembly|null;holder:T.Group;revision:number;focused:boolean};
...
export const AlienViewer=forwardRef<ViewerHandle,Props>(function AlienViewer(props,ref){
 ...
 const focus=()=>{const e=engine.current;if(!e?.assembly)return;e.focused=true;e.holder.rotation.y=0;const region=latest.current.selected;let box=regionBounds(e.assembly.root,region);if(box.isEmpty()&&region==='eye')box=regionBounds(e.assembly.root,'head');frameRegion(e.camera,e.orbit,box);};
 const look=(z:number)=>{const e=engine.current;if(e){e.focused=false;e.holder.rotation.y=0;e.orbit.minDistance=5.2;e.camera.position.set(0,2.8,z);e.orbit.target.set(0,1.95,0);e.orbit.update();}};
 useImperativeHandle(ref,()=>({front:()=>look(8.5),back:()=>look(-8.5),focus,async exportFile(format){
 ...
 useEffect(()=>{
  ...
  const e:Engine={scene,camera,renderer,orbit,assembly:null,holder,revision:0,focused:true};engine.current=e;setStarted(true);
  const resize=()=>{const box=mount.current?.getBoundingClientRect();if(box?.width&&box.height){renderer.setSize(box.width,box.height,false);camera.aspect=box.width/box.height;camera.updateProjectionMatrix();if(e.focused)focus();}};const observer=new ResizeObserver(resize);observer.observe(mount.current);resize();
