...
export function parseRecipe(raw:string):Design{const d=JSON.parse(raw);d.pupil??='round';d.pupilSize??=1;d.detail??=1;d.coach??='supportive';d.fingers??=4;d.toes??=3;d.eyeLayout??='single';if(!Number.isInteger(d.fingers)||d.fingers<2||d.fingers>6||!Number.isInteger(d.toes)||d.toes<1||d.toes>6||!Object.hasOwn(EYE_LAYOUTS,d.eyeLayout)||d.version!==1||!d.styles||!REGIONS.every(r=>Number.isInteger(d.styles[r])&&d.styles[r]>=0&&d.styles[r]<STYLES.length)||!['open','sleepy','wide'].includes(d.eye)||!Number.isFinite(d.fur)||d.fur<.65||d.fur>1.4||!Number.isFinite(d.iris)||d.iris<.7||d.iris>1.25||!PUPILS.some(p=>p[0]===d.pupil)||!Number.isFinite(d.pupilSize)||d.pupilSize<.6||d.pupilSize>1.15||!Number.isFinite(d.detail)||d.detail<.5||d.detail>1.5||!COACHES.some(c=>c.id===d.coach))throw Error('Choose a valid MYR5 recipe.');return {version:1,styles:Object.fromEntries(REGIONS.map(r=>[r,d.styles[r]])) as Design['styles'],eye:d.eye,fur:d.fur,iris:d.iris,pupil:d.pupil,pupilSize:d.pupilSize,detail:d.detail,coach:d.coach,fingers:d.fingers,toes:d.toes,eyeLayout:d.eyeLayout};}
