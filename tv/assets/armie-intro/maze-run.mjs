...
export function createRun(seed = 0, assisted = false) {
  return {seed, assisted, distance:0, time:0, lane:0, jumpTime:0, height:0, turn:null, paused:false, status:'running', events:courseFor(seed).map(e=>({...e, cleared:false}))};
}
...
export function input(run, action) {
  ...
  if(action === 'jump') { if(run.jumpTime === 0) run.jumpTime = .96; return; }
...
export function safe(run, event) {
  ...
  return run.lane!==event.lane || event.type==='rubble' && run.height>.65;
}
...
export function hint(run) {
  ...
  if(!e || left>11) return 'Keep moving · two lanes';
  if(e.type==='turn') return `${e.direction==='left'?'←':'→'} TURN ${e.direction.toUpperCase()}${run.turn===e.direction?' · READY':''}`;
  if(e.type==='crack') return left<3?'JUMP NOW ↑':'CRACK AHEAD · JUMP ↑';
  return `${e.type==='wall'?'SLIDING WALL':'FALLEN STONE'} · ${e.lane===0?'MOVE RIGHT →':'← MOVE LEFT'}`;
}
