...
import { DIGITS, type Digit, type Pose } from './poses.ts';

// Rest centerlines are shared with the anatomical sculpt (converted to Y-up).
export const REST_SOURCE:Record<Digit,number[][]>={
...
type RigProfile='standard'|'baby';
const makeBones=(landmarks:THREE.Vector3[][])=>landmarks.map(points=>points.slice(0,3).map((p,i)=>{
...
export function poseMatrices(pose:Pose,profile:RigProfile='standard'):THREE.Matrix4[] {
  ...
  for(let d=0;d<5;d++){
    ...
    for(let i=0;i<3;i++){
      const bone=bones[d][i];let q:THREE.Quaternion,p:THREE.Vector3;
      if(i===0){
  ...
  const rotation=new THREE.Matrix4().makeRotationFromEuler(new THREE.Euler(...pose.rotation.map(v=>v*rad) as [number,number,number]));
  return matrices.map(m=>rotation.clone().multiply(m));
}
export function fingerTip(pose:Pose,digit:Digit):THREE.Vector3 {
  const index=DIGITS.indexOf(digit);return rest[index][3].clone().applyMatrix4(poseMatrices(pose)[index*3+3]);
}

function influences(x:number,y:number,z:number,profile:RigProfile='standard'):[number,number][] {
  ...
  const candidates:{score:number;gain:number;d:number;i:number;t:number}[]=[];
  for(let d=0;d<5;d++){
  ...
  const chosen=candidates.slice(0,2),total=chosen.reduce((s,c)=>s+c.score,0),weights=new Map<number,number>();
  const add=(id:number,w:number)=>weights.set(id,(weights.get(id)??0)+w);
...
type BoundMesh={mesh:THREE.Mesh;profile:RigProfile;positions:Float32Array;normals:Float32Array;ids:Uint8Array;weights:Float32Array;inverse:THREE.Matrix4;inverseNormal:THREE.Matrix3};
export type HandRig={group:THREE.Group;meshes:BoundMesh[];dispose:()=>void};
export function bindHand(group:THREE.Group):HandRig {
  group.updateMatrixWorld(true);const bound:BoundMesh[]=[];
  group.traverse(obj=>{
    ...
    const profile:RigProfile=obj.userData.rig_profile==='baby'?'baby':'standard';
    // Every strand or scale shares its attachment weights, so fingers cannot tear it apart.
    const furRoots=obj.geometry.getAttribute('_fur_root')??obj.geometry.getAttribute('_surface_root');
    let rootX=NaN,rootY=NaN,rootZ=NaN,rootWeights:[number,number][]=[];
