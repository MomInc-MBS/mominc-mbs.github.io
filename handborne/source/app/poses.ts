export const DIGITS = ['index','middle','ring','pinky','thumb'] as const;
export type Digit = typeof DIGITS[number];
export type FingerPose = [number,number,number,number]; // MCP, PIP, DIP flexion; spread, degrees
export type Pose = { id:string; name:string; description:string; fingers:Record<Digit,FingerPose>; opposition:number; rotation:[number,number,number] };
const relaxed:FingerPose=[0,0,0,0],straight:FingerPose=[-3,-8,-7,0],fold:FingerPose=[72,88,42,0];
const thumbIn:FingerPose=[0,6,8,-60];
function pose(id:string,name:string,description:string,fingers:Partial<Record<Digit,FingerPose>>={},opposition=0,rotation:[number,number,number]=[0,0,0]):Pose {
...
export function getPose(id:string):Pose { const found=POSES.find(p=>p.id===id);if(!found)throw new Error('Unknown hand pose.');return found; }
export function validatePose(value:unknown):string {if(typeof value!=='string')throw new Error('Choose a valid hand pose.');getPose(value);return value;}
export function mixPoses(a:Pose,b:Pose,t:number):Pose {
  if(t>=1)return b;
  const lerp=(x:number,y:number)=>x+(y-x)*t;
  return {...b,fingers:Object.fromEntries(DIGITS.map(d=>[d,a.fingers[d].map((v,i)=>lerp(v,b.fingers[d][i]))])) as Record<Digit,FingerPose>,opposition:lerp(a.opposition,b.opposition),rotation:a.rotation.map((v,i)=>v+((b.rotation[i]-v+540)%360-180)*t) as Pose['rotation']};
}
