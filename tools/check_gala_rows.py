...
with sync_playwright() as pw:
 ...
 for width,height in [(1440,1000),(390,844)]:
  ...
  p.locator('[data-section="pet"]').click();expect(p.locator('#category-name')).to_have_text('Pet');p.locator('#options button').nth(39).click();expect(p.locator('#options button').nth(39)).to_have_attribute('aria-pressed','true')
  box=p.locator('#avatar').bounding_box();assert box['y']>=0 and box['y']+box['height']<height-60,box
  choice=p.locator('#options button').nth(39).bounding_box();assert choice['y']>=box['y']+box['height'],(box,choice)
