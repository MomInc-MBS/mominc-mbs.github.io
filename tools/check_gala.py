...
with sync_playwright() as pw:
 ...
 p.goto('http://127.0.0.1:8898/gala/');expect(p.locator('#sections button')).to_have_count(18);expect(p.locator('#options button')).to_have_count(20)
 ...
 chosen_id=int(p.locator('#options button').nth(1).get_attribute('data-piece'));p.locator('#options button').nth(1).click();p.locator('#guest-name').fill('Lady Quasar');p.locator('#save').click();expect(p.locator('#looks button')).to_have_count(1)
 p.reload();expect(p.locator('#guest-name')).to_have_value('Lady Quasar');assert p.evaluate("JSON.parse(localStorage.getItem('mominc-avatar-v1')).parts.body")==chosen_id
 ...
 print('PASS 20 offered choices, all 680 legacy renders, persistence, undo/redo, export/import, PNG, mobile and gala avatar',flush=True);b.close()
