...
with sync_playwright() as pw:
 ...
 g=popup.value;g.wait_for_url('**/gala/');expect(g.locator('#avatar')).to_be_visible();g.locator('#join').click();g.wait_for_url('**/play/goon/');expect(g.frame_locator('#ggGame').locator('canvas')).to_be_visible();frame=g.locator('#ggFrame').bounding_box();assert frame['width']>1350,frame
