...
with sync_playwright() as pw:
 ...
 p.locator('[data-network-launch] a').click();assert len(b.contexts[0].pages)==1,'The Gala opens in this tab'
 g=p;g.wait_for_url('**/gala/');expect(g.locator('#avatar')).to_be_visible();g.locator('#join').click();g.wait_for_url('**/play/goon/');expect(g.frame_locator('#ggGame').locator('canvas')).to_be_visible();frame=g.locator('#ggFrame').bounding_box();assert frame['width']>1350,frame
 g.locator('.exit').click();g.wait_for_url('**/tv/?ch=goon')
 p.goto('http://127.0.0.1:8898/games/goon/');expect(p.locator('.network-launch video')).to_be_visible();assert p.locator('#play').get_attribute('target') is None
